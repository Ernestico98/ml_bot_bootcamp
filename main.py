...
def run_bot(model):
	...
	def send_welcome(message):
		msg = bot.reply_to(message, 'Welcome, state your city code in the format "city_{$number}"')
		bot.register_next_step_handler(msg, process_city_code)

	def process_city_code(message):
		city = message.text
		data['city'] = city
		msg = bot.reply_to(message, 'Insert  city development index (real number in range[0,1]) ')
	...
	def process_gender(message):
		...
		msg = bot.reply_to(message, "Relevant experience ",reply_markup=markup)
	...
	def process_relevant_exp(message):
		...
		msg = bot.reply_to(message, "University enrollment ",reply_markup=markup)
	...
	def process_enrolment(message):
		...
		markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
		markup.add('Graduate', 'Masters', 'High School', 'Phd', 'Primary School')  
	...
	def process_education_level(message):
		...
		markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
		markup.add('STEM', 'Business Degree', 'Arts', 'Humanities', 'No Major','Other')  
	...
	def process_education_major(message):
		...
		markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
		markup.add('<10', '10/49', '50-99', '100-500', '500-999', '1000-4999', '5000-9999', '10000+')  
	...
	def process_time_experience(message):
		...
		markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
		markup.add('Pvt Ltd', 'Funded Startup', 'Early Stage Startup', 'Other','Public Sector', 'NGO')  
	...
	def predict_with_model():
		...
		return f'Your probability of staying with us is {prob[0, 1]}'
